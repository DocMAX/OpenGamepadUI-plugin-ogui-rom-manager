...
func _ready():
    ...
    $HBoxContainer/VBoxContainer/CreateNewParserTab.connect("pressed", Callable(self, "_on_create_new_parser_pressed"))
...
func _on_create_new_parser_pressed():
    print("Create New Parser button pressed")
    _add_new_tab_in_edit_mode()

func _add_new_tab_in_edit_mode():
    ...
    var tab_index = parser_tabs.size()
    parser_tabs.append(tab_container)
...
func _on_confirm_parser_name(tab_index):
    var tab_container = parser_tabs[tab_index]
    ...
    _finalize_tab(tab_index, parser_name)
...
func _finalize_tab(tab_index, parser_name):
    ...
    var tab_content = _create_tab_content(parser_name, tab_index)
    tab_contents.append(tab_content)
    tab_titles.append(parser_name)
    
    var panel_vbox = $HBoxContainer/PanelContainer/VBoxContainer
    panel_vbox.add_child(tab_content)
    tab_content.hide()
    
    _set_active_tab(tab_index)

func _create_tab_content(parser_name, tab_index):
    ...
    add_dir_button.connect("pressed", Callable(self, "_on_add_directory_pressed").bind(dirs_list))
...
func _on_parser_title_changed(new_text: String, tab_index: int):
    tab_titles[tab_index] = new_text
    var tab_container = parser_tabs[tab_index]
    var tab_button = tab_container.get_child(0) as Button
    tab_button.text = new_text

func _on_add_directory_pressed(dirs_list: VBoxContainer):
...
func _set_active_tab(tab_index):
    print("Setting active tab: ", tab_index)
    if active_tab_index == tab_index:
        return
    
    if active_tab_index >= 0 and active_tab_index < tab_contents.size():
        var prev_content = tab_contents[active_tab_index]
    ...
    if tab_index >= 0 and tab_index < tab_contents.size():
        var new_content = tab_contents[tab_index]
...
func _on_close_tab(tab_index):
    ...
    if tab_index < 0 or tab_index >= parser_tabs.size():
        return
    
    var tab_container = parser_tabs[tab_index]
    $HBoxContainer/VBoxContainer.remove_child(tab_container)
    tab_container.queue_free()
    
    if tab_index < tab_contents.size():
        var content = tab_contents[tab_index]
        if content.get_parent():
            content.get_parent().remove_child(content)
        content.queue_free()
    
    parser_tabs.remove_at(tab_index)
    tab_contents.remove_at(tab_index)
    tab_titles.remove_at(tab_index)
    
    for i in range(parser_tabs.size()):
        var tab = parser_tabs[i]
